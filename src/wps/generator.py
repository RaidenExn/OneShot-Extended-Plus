...
class NetworkAddress:
    def __init__(self, mac):
        if isinstance(mac, int):
            if mac < 0:
                raise ValueError("MAC address as integer cannot be negative")
            self._INT_REPR = mac
            ...
            try:
                ...
                raise ValueError(f"Invalid MAC address string: {mac}")
        else:
            raise TypeError(f"Unsupported type for NetworkAddress: {type(mac)}")
...
class WPSpin:
    ...
    def promptPin(self, bssid: str):
        ...
        if len(pins) > 1:
            ...
            while True:
                pin_no_str = input('Select the PIN: ')
                try:
                    pin_no = int(pin_no_str)
                    if 1 <= pin_no <= len(pins):
                        pin = pins[pin_no - 1]['pin']
                        break
                    else:
                        print(f'Invalid number. Please enter a number from 1 to {len(pins)}.')
                except ValueError:
                    print('Invalid input. Please enter a number.')
        else:
            pin_data = pins[0]
            print(f"[*] The only probable PIN is selected: {pin_data['name']}")
            pin = pin_data['pin']
    ...
    def _suggest(self, bssid: str) -> list:
        ...
        res = []
        for algo_id, masks in self._ALGO_SUGGESTIONS.items():
            if mac.startswith(masks):
                res.append(algo_id)
        return res

    @staticmethod
    def _pin24(bssid: NetworkAddress):
        return bssid.INTEGER & 0xFFFFFF

    @staticmethod
    def _pin28(bssid: NetworkAddress):
        return bssid.INTEGER & 0xFFFFFFF

    @staticmethod
    def _pin32(bssid: NetworkAddress):
        return bssid.INTEGER % 0x100000000

    @staticmethod
    def _pinDLink(bssid: NetworkAddress):
        ...
        pin %= 10_000_000
        if pin < 100_000:
            pin += ((pin % 9) * 100_000) + 100_000
        return pin

    @staticmethod
    def _pinASUS(bssid: NetworkAddress):
        b = [int(i, 16) for i in str(bssid).split(':')]
        pin_str = ''
        for i in range(7):
            pin_str += str((b[i % 6] + b[5]) % (10 - (i + b[1] + b[2] + b[3] + b[4] + b[5]) % 7))
        return int(pin_str)

    @staticmethod
    def _pinAirocon(bssid: NetworkAddress):
        b = [int(i, 16) for i in str(bssid).split(':')]
        pin = ((b[0] + b[1]) % 10)\
        + (((b[5] + b[0]) % 10) * 10)\
        + (((b[4] + b[5]) % 10) * 100)\
    ...
    def _pinDLink1(self, bssid: NetworkAddress):
        bssid.INTEGER += 1
    ...
    def _generate(self, algo: str, bssid: str):
        ...
        pin_gen_func = self.ALGOS[algo]['gen']
        pin = pin_gen_func(mac)

        if algo == 'pinEmpty':
            return pin

        pin_int = int(pin) % 10_000_000
        pin_str = str(pin_int) + str(self.checksum(pin_int))
